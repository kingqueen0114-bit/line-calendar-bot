...
**タスク/予定/メモの判定（actionがcreateの場合のみ）:**
- メモ: 「メモ」キーワードが**明示的に含まれている**場合、または情報の記録・備忘録の場合
  例: 「メモ 買い物リスト: 牛乳、卵、パン」→ action="create", type="memo", text="買い物リスト: 牛乳、卵、パン"
  例: 「メモ 田中さんの電話番号 090-1234-5678」→ action="create", type="memo", text="田中さんの電話番号 090-1234-5678"
  例: 「覚書 次回の会議の議題は予算について」→ action="create", type="memo", text="次回の会議の議題は予算について"
  - 日付や時刻がなく、単なる情報の記録の場合はメモ
  - 「覚書」「記録」「忘れないように」などもメモとして扱う
- タスク: 「タスク」「やること」「TODO」キーワードが**明示的に含まれている**場合、またはアクションが必要な場合
  ...
  例: 「やること 部屋の掃除」→ action="create", type="task", title="部屋の掃除"
  **重要**: タイトルに「キャンセル」「削除」「変更」が含まれていても、「タスク」があればaction="create"
- 予定: 日時が含まれている場合、またはイベント・アポイントメントの場合
  - 時刻がある場合: 通常の予定
  - 時刻がない場合: 終日予定として登録
  例: 「明日 会議」→ 終日予定
  例: 「3月1日 テスト」→ 終日予定
